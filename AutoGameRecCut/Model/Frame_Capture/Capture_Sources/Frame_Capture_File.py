...
class Frame_Capture_File(IFrameCaptureInterface):
    """
    Robust frame capture class:
   - very Slow
   - Maps pts_time <-> frame index with tolerance
   - Stores recent timestamps for external logging
   - Base resync (base_pts/base_frame) to compensate for drift
   - Optional GPU (torch) path for later YOLOv8 processing
    """

    PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..",".."))
    FFPROBE_BIN = os.path.join(PROJECT_ROOT, "ffmpeg-7.1.1-full_build", "bin", "ffprobe.exe")
